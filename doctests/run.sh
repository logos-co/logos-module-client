#!/usr/bin/env bash
#
# Execute every logos-module-client doc-test end-to-end and regenerate its Markdown.
#
...
# published flake. Each spec's `github:logos-co/logos-module-client{release}` URL is pinned to
...
# to logos-co/logos-module-client. A local-only / uncommitted HEAD won't resolve; export
# COMMIT="" (or push first) in that case.
COMMIT="${COMMIT-$(git rev-parse HEAD)}"
RELEASE_FOR=()
if [ -n "${COMMIT}" ]; then
  RELEASE_FOR=(--release-for "logos-module-client=${COMMIT}")
  echo "==> Pinning logos-module-client to ${COMMIT}"
else
  echo "==> COMMIT empty; building logos-module-client from latest"
fi
